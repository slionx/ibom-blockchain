...
async function main(){
  ...
  const amountStr = arg('--amount', '0.1');
  if (!workStr) throw new Error('Missing --work <WorkPDA>');
  const lamports = BigInt(Math.floor((Number(amountStr)||0)*1e9));
  ...
  console.log('Payer   :', authority.toBase58());
  console.log('Pool PDA:', poolPda.toBase58());
  await program.methods
    .fundSol(new anchor.BN(lamports.toString()))
    .accounts({ payer: authority, pool: poolPda, systemProgram: SystemProgram.programId })
    .rpc();
  console.log('fund_sol done:', amountStr,'SOL');
}
